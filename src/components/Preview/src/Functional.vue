...
import resumeSvg from '@/assets/images/preview/resume.svg'
import rotateSvg from '@/assets/images/preview/rotate.svg'
import scaleSvg from '@/assets/images/preview/scale.svg'
import unScaleSvg from '@/assets/images/preview/unscale.svg'
import unRotateSvg from '@/assets/images/preview/unrotate.svg'
import LeIcon from '@/components/Icon.vue'

type ImgStatus = 'loading' | 'success' | 'fail'
interface ImgState {
	currentUrl: string
	imgScale: number
	imgRotate: number
...
export default defineComponent({
	...
	setup(props, { expose, emit }) {
		...
		const getScaleStep = computed(() => {
			...
			if (scaleStep ?? scaleStep < 100) return scaleStep / 100
		...
		const handleMaskClick = (e: MouseEvent) => {
			if (props.maskClosable && e.target && (e.target as HTMLDivElement).classList.contains(`${prefixCls}-content`)) handleClose(e)
		}
		...
		const renderClose = () => {
			...
			return (
				<div class={`${prefixCls}__close`} onClick={handleClose}>
					<LeIcon class={`${prefixCls}__close-icon`} icon="ep:close" />
				</div>
			)
		}
		...
		const renderArrow = (direction: 'left' | 'right') => {
			...
			return (
				<div class={[`${prefixCls}__arrow`, direction]} onClick={() => handleChange(direction)}>
					...
					<LeIcon icon={`${direction === 'left' ? 'ep:arrow-left' : 'ep:arrow-right'}`} />
				</div>
			)
		}
				...
				imgState.show && (
					<div class={prefixCls} ref={wrapElRef} onMouseup={handleMouseUp} onClick={handleMaskClick}>
						<div class={`${prefixCls}-content`}>
							...
							{/*    `${prefixCls}-image`, */}
							{/*    { */}
							{/*      hidden: imgState.status !== 'loading', */}
							{/*    }, */}
							{/*  ]} */}
							{/* /> */}
							<img
								style={unref(getImageStyle)}
								class={[`${prefixCls}-image`, imgState.status === 'success' ? '' : 'hidden']}
								ref={imgElRef}
